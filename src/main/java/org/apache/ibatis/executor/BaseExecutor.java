...
public abstract class BaseExecutor implements Executor {
   ...
   * 延迟加载队列。一个基于链接节点的无界线程安全队列。此队列按照 FIFO（先进先出）原则对元素进行排序。 当多个线程共享访问一个公共 collection 时，ConcurrentLinkedQueue
   * 是一个恰当的选择。此队列不允许使用 null元素。该变量主要是存储一些可以延时加载的变量对象，且可供多线程使用
   */
  protected ConcurrentLinkedQueue<DeferredLoad> deferredLoads;
  // 一级缓存，用于缓存该 Executor对象查询结果集映射得到的结果对象，此类中成员如下
  /*
   * public class PerpetualCache implements Cache { private final String id; private Map<Object, Object> cache = new
   * HashMap<>();
   */
  ...
  /**
   * 关闭当前执行器，如果需要回滚，就执行回滚操作 需要把相应的资源（比如事务transaction等），同步关闭 把其他变量设置成NULL
   */
  @Override
  ...
  /**
   * update/insert/delete请求到达SqlSession都会调用此方法 从语义的角度，insert、update、delete都是属于对数据库的行进行更新操作
   ...
   * 该方法处理了通用的情况，真正实现由子类通过实现doUpdate方法完成。该方法实现了： 判断该执行器是否已经关闭，如果关闭，就直接抛出异常 因为该操作是更新操作，所以该操作会让其中的缓存就全部失效，即清理缓存
  ...
  /**
   * SqlSession.selectList会调用此方法 在该方法中，首先判断当前执行器是否已经关闭；然后再根据queryStack查询层数和flushCache属性判断，
   * 是否需要清除一级缓存；然后再判断结果处理器resultHandler是否为空，为空的话，尝试从缓存中查询数据， 否则直接为空，后续从数据库查询数据；然后再根据缓存中是否有数据，如果存在数据，
  ...
  public <E> List<E> query(MappedStatement ms, Object parameter, RowBounds rowBounds, ResultHandler resultHandler,
    ...
    try {
      ...
      if (list != null) {
        /*
         * 针对存储过程调用的处理 其功能是 在一级缓存命中时，获取缓存中保存的输出类型参数， 并设到用户传入的实参（ parameter ）对象中。
         */
    ...
    if (queryStack == 0) {
      ...
      /**
       * LocalCacheScope为STATEMENT的时候，会清理缓存。但是结果 list已经从localcache拿到了，然后直接返回了。再清理缓存，还有啥意义？ 这个问题不应该割裂的看，应该从第一次查询来看。
       * 第一次查询的时候，list必然为空，然后从db查询出结果放到localCache。 放完后，发现是statement级别，需要再清理掉缓存，那么cache又变成了空。
  ...
  /**
   * createCacheKey()方法 创建缓存中使用的key，即CacheKey实例对象。创建的CacheKey实例对象，由下列参数决定其唯一性， 即相同的查询，相关的参数，生成的CacheKey实例唯一且不变。依据的参数如下：
   * 1、MappedStatement实例的ID 2、RowBounds实例的offset和limit属性 3、BoundSql实例的sql属性 4、查询条件的参数值 5、连接数据库的环境ID
